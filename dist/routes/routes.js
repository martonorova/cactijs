...
module.exports = function (app) {
    ...
    app.get('/', mainRedirectMW());
    // Fooldal, a cserepek listaja
    app.get('/dashboard', getPotListMW(objRepo), getDayCounterMW(objRepo), renderMW(objRepo, 'potlist'));
    // Lepteti a nap szamlalot
    app.get('/dashboard/nextday', getCactusListMW(objRepo), degradeBuggyCactiMW(objRepo), getCactusListMW(objRepo), bugCactiMW(objRepo), 
    // resetDayCounterMW(objRepo),
    incrementDayCounterMW(objRepo), mainRedirectMW());
    // Rendereli a cserep hozzaadasa formot
    app.get('/pots/new', getSizeOptionsMW(), getColorOptionsMW(), renderMW(objRepo, 'newpot'));
    ...
    app.get('/pot/:potid/repaint', getPotByIdMW(objRepo), getColorOptionsMW(), renderMW(objRepo, 'repaint'));
    // Atfesti a cserepet
    app.post('/pot/:potid/repaint', getPotByIdMW(objRepo), repaintPotMW(objRepo));
    // Kitorli a cserepet
    app.get('/pot/:potid/delete', deletePotMW(objRepo));
    // Rendereli az uj kaktusz formot
    app.get('/pot/:potid/plant', getCactusTypesMW(), renderMW(objRepo, 'newcactus'));
    // Letrehoz egy uj kaktuszt
    app.post('/pot/:potid/plant', getPotByIdMW(objRepo), createCactusMW(objRepo));
    // Rendereli a kaktusz info oldalt
    app.get('/cactus/:cactusid', getCactusMW(objRepo), renderMW(objRepo, 'cactusinfo'));
    // Kaktusz ontozes
    app.get('/cactus/:cactusid/water', getCactusMW(objRepo), getPotByCactusMW(objRepo), cactusGrowMW(objRepo), renderMW(objRepo, 'cactusinfo'));
    // Bogar eltavolitasa a kaktuszrol
    app.get('/cactus/:cactusid/killbug', getCactusMW(objRepo), unbugCactusMW(objRepo));
};
